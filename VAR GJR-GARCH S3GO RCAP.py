...
# Load data from Excel (specify the Excel file and sheet)
excel_file = '/Users/elvinhuang/Desktop/Placement/Regression Analysis.xlsx'
sheets_to_process = ['S3GO Y', 'RCAP Y']  # Replace with the names of the selected sheets
...
for sheet_name in sheets_to_process:
    ...
    # Extract the selected columns for the two time series
    ts1 = df['Flow']
    ts2 = df['Return']

    # Combine the two time series into one DataFrame
    data = pd.concat([ts1, ts2], axis=1)
    data.columns = ['Flow', 'Return']
    ...
    residuals = final_results.resid['Flow']
    print(residuals)

    # Forecast future values
    forecast_steps = 4  # Adjust this for the number of time steps you want to forecast
    mean_forecasts = final_results.forecast(data.values[-best_p:], steps=forecast_steps)
    ...
    print("Linear Equation for Flow:")
    ...
    upper_bound = mean_forecasts[:, 0] + 1.96 * forecasted_std_dev
    lower_bound = mean_forecasts[:, 0] - 1.96 * forecasted_std_dev
    ...
    fitted_values = final_results.fittedvalues['Flow']
    combined_dates =start_date_for_fitted.tolist() + forecast_index.tolist()
    combined_flow = fitted_values.tolist() + mean_forecasts[:, 0].tolist()
    ...
    plt.plot(df['Month'], df['Flow'], label='Actual Flow', color='blue')
    plt.fill_between(forecast_index, lower_bound, upper_bound, color='pink', alpha=1, label='Prediction Interval')
    plt.plot(forecast_index, mean_forecasts[:, 0], color='red')
    plt.xlabel('Month')
    plt.ylabel('Value')
